// Command carrier is a mock MCP server exposing link_carrier_account.
// Deliberately its own server, not a second tool on shipping-mcp: agentgateway's
// entElicitation gates at the whole-Backend level, so co-locating this tool would
// force carrier OAuth consent before the existing Stage 3/4/6/7 A2A chain (which
// calls shipping's get_shipment_status) could run. A dedicated Backend scopes the
// gate to Stage 9 alone.
package main
